...
$am_var_author_avatar       = $bst_fields['am_var_author_avatar'] ?? null;

$am_var_post_catagories = get_categories( $am_var_post_id );
	...
	<div class="wrapper">
		...
		<div class="blog-nav">
			<div class="nav-ctn d-flex justify-content-center flex-wrap">
				<?php if ( $am_var_post_catagories ) { ?>
					<ul>
						<?php foreach ( $am_var_post_catagories as $am_var_category ) { ?>
							<li><a href="<?php echo esc_url( get_category_link( $am_var_category ) ); ?>"><?php echo esc_html( $am_var_category->name ); ?></a></li>
						<?php } ?>
					</ul>
				<?php } ?>
		...
		<div class="am-s11"></div>
		<?php
		if ( $am_var_tblgho_feature_post ) {
			?>
			<?php

			foreach ( $am_var_tblgho_feature_post as $am_var_post_id ) {
				...
				if ( ! $am_var_post_image ) {
					$am_var_post_image = get_template_directory_uri() . '/assets/build/images/admin/defaults/default-image.webp';
				}
				?>
					...
					<div class="resources-inner d-flex flex-wrap align-items-center justify-content-between">
						...
						if ( $am_var_post_image ) {
							...
							<div class="rc-post-img post-image rs-view-100">
									<a href="<?php echo esc_url( $am_var_post_parmalink ); ?>">
									<img src="<?php echo esc_url( $am_var_post_image ); ?>">
									</a>
							</div>
						<?php } ?>
						<div class="post-content rs-view-100">

							<?php if ( $am_var_post_tags ) { ?>
								<div class="post-tag">
									<?php
									foreach ( $am_var_post_tags as $am_var_post_tag ) {
										$am_var_post_cat_name = $am_var_post_tag->name;
										$am_var_post_cat_link = get_category_link( $am_var_post_tag->term_id );
										?>
										<a href="<?php echo esc_url( $am_var_post_cat_link ); ?>">
											<?php echo esc_html( $am_var_post_cat_name ); ?>
										</a>
									<?php } ?>
								</div>
							<?php } ?>

							<?php if ( $am_var_post_title ) { ?>
								<div class="post-box-title">
									<h2><a href="<?php echo esc_url( $am_var_post_parmalink ); ?>"><?php echo esc_html( $am_var_post_title ); ?></a></h2>
								</div>
							<?php } ?>
							<!-- post excerpt -->
							<?php if ( $am_var_post_excerpt ) { ?>
								<div class="post-box-excerpt">
									<p>
										<?php echo esc_html( $am_var_post_excerpt ); ?>
									</p>
								</div>
							<?php } ?>
								...
								<div class="post-author-ctn d-flex">
									<?php if ( $am_var_author_avatar ) { ?>
										<div class="post-author-img"
											style="background-image: url(<?php echo esc_url( $am_var_author_avatar ); ?>); width:50px; height:50px; background-size:cover">
										</div>
									<?php } ?>
									<div class="author-meta">
										<?php if ( $am_var_author_name ) { ?>
											<div class="post-author-name"><?php echo esc_html( $am_var_author_name ); ?></div>
										<?php } ?>
										<?php if ( $am_var_post_date ) { ?>
											<div class="post-meta-date"><?php echo esc_html( $am_var_post_date ); ?></div>
										<?php } ?>
									</div>
								</div>
							</div>
